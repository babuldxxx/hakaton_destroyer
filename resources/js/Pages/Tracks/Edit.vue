...
const form = useForm({
    ...
    label_share_percent: props.song.label_share_percent ?? 0,
    cover: null,
    mp3: null,
    wav: null,
    _method: 'put',
    authors: props.song.song_authors?.map(a => ({
        ...
        share_percentage: parseFloat(String(a.share_percentage).replace(',', '.')) || 0,
        role: a.role,
        rights_type: a.rights_type ?? 'author_rights', // ← важно!
    })) || [],
...
function addAuthor() {
    form.authors.push({
        ...
        share_percentage: 0,
        role: 'author',
        rights_type: 'author_rights', // ← по умолчанию
    })
}
...
function submit() {
    // Очистка долей: заменяем запятую на точку и приводим к числу
    form.authors.forEach(a => {
        if (typeof a.share_percentage === 'string') {
            a.share_percentage = parseFloat(a.share_percentage.replace(',', '.')) || 0;
        } else if (typeof a.share_percentage === 'number') {
            a.share_percentage = parseFloat(a.share_percentage) || 0;
        }
    })
                ...
                <div class="space-y-3">
                    ...
                    <div v-for="(author, index) in form.authors" :key="index"
                        ...
                        <div class="md:col-span-2">
                            <input v-model="author.share_percentage" type="number" min="0" max="100" step="any" placeholder="Доля %" required
                                   class="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white text-sm focus:border-indigo-500 outline-none" />
                        ...
                        <input type="hidden" v-model="author.rights_type" />
