...
function submit() {
  form.post('/tracks', {
    forceFormData: true,
    onSuccess: () => {
      // после успешного сохранения вернётся на список
    }
  })
}
    ...
    <div class="p-6 md:p-10 max-w-4xl mx-auto text-white">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          <!-- Вывод ошибок по авторам -->
          <div v-if="form.errors['authors.0.artist_id']" class="text-red-400 text-sm">
            Укажите артиста
          </div>
          <div v-if="form.errors['authors.0.share_percentage']" class="text-red-400 text-sm">
            Укажите корректную долю (0–100)
          </div>
        </div>
