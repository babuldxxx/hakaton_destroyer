...
const form = useForm({
  ...
  mp3: null,
  wav: null,
  // ключевой момент: share_percentage число 0, а не пустая строка
  authors: [{ artist_id: '', share_percentage: 0, role: 'author', rights_type: 'author_rights' }],
})

function addAuthor() {
  form.authors.push({ artist_id: '', share_percentage: 0, role: 'author', rights_type: 'author_rights' })
}

function removeAuthor(index) {
  if (form.authors.length <= 1) return // не даём удалить последнего
    ...
    <div class="p-6 md:p-10 max-w-3xl mx-auto text-white">
      ...
      <form @submit.prevent="submit" class="space-y-5">
        ...
        <div>
          ...
          <textarea v-model="form.lyrics" rows="3"
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <label class="block text-sm text-gray-400">Авторы и доли <span class="text-red-500">*</span></label>
            <button type="button" @click="addAuthor" class="text-sm text-blue-400 hover:text-blue-300 transition">+ Добавить автора</button>
          </div>
          ...
          <div v-for="(author, index) in form.authors" :key="index"
            ...
            <div class="md:col-span-2">
              <input v-model.number="author.share_percentage" type="number" min="0" max="100" required
                class="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white text-sm focus:border-indigo-500 outline-none" />
        ...
        <div class="flex items-center gap-3 pt-2">
          ...
          <a :href="route('tracks.index')" class="text-sm text-gray-400 hover:text-white transition">Отмена</a>
        </div>
      </form>
    </div>
  </AuthenticatedLayout>
