  ...
  switch (selectedPeriod.value) {
    case 'week':
      return {
        ...
        datasets: [{ ...common, data: [120000,190000,150000,220000,280000,350000,310000] }]
      }
    case 'month':
      return {
        labels: ['1','5','10','15','20','25','30'],
        datasets: [{ ...common, data: [450000,520000,480000,610000,580000,720000,680000] }]
      }
    case 'quarter':
      return {
        labels: ['Нед 1','Нед 2','Нед 3','Нед 4','Нед 5','Нед 6','Нед 7','Нед 8','Нед 9','Нед 10','Нед 11','Нед 12'],
        datasets: [{ ...common, data: [1200000,1350000,1400000,1550000,1800000,2000000,2200000,2150000,2600000,3000000,3500000,3800000] }]
      }
    case 'half':
      return {
        labels: ['Янв','Фев','Мар','Апр','Май','Июн'],
        datasets: [{ ...common, data: [2100000,2350000,2500000,2700000,2900000,3200000] }]
      }
    case 'year':
    default:
      return {
        labels: ['Янв','Фев','Мар','Апр','Май','Июн','Июл','Авг','Сен','Окт','Ноя','Дек'],
        datasets: [{ ...common, data: [2100000,2350000,2500000,2700000,2900000,3200000,3450000,3600000,3800000,3900000,4300000,4500000] }]
      }
  }
})

/* ─── Donut: доходы по площадкам ─── */
const platformData = {
  labels: ['VK Music', 'Apple Music', 'Яндекс.Музыка', 'Spotify', 'Другие'],
  datasets: [{
    data: [5420000, 4130000, 2800000, 1537000, 600000],
        ...
        <div class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          >
            ...
            <div class="mt-6 space-y-2">
              <div 
                v-for="p in platformLegend" 
                :key="p.name"
                class="flex items-center justify-between rounded-lg px-3 py-2"
                style="background-color: #0B0E14;"
              >
                <div class="flex items-center gap-2">
                  <span class="h-3 w-3 rounded-full" :style="{ backgroundColor: p.color }"></span>
                  <span class="text-sm" style="color: #94A3B8;">{{ p.name }}</span>
                </div>
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  <p class="truncate text-sm font-semibold text-white">{{ tr.title }}</p>
                  <p class="truncate text-xs" style="color: #64748B;">{{ tr.artist }}</p>
                </div>

                <!-- Площадка -->
                <span
                  class="hidden shrink-0 rounded-md px-2 py-1 text-[10px] font-bold uppercase tracking-wider sm:inline-block"
                  :class="{
                    'bg-blue-500/10 text-blue-400': tr.source === 'VK Music',
                    'bg-pink-500/10 text-pink-400': tr.source === 'Spotify',
                    'bg-violet-500/10 text-violet-400': tr.source === 'Apple Music',
                    'bg-emerald-500/10 text-emerald-400': tr.source === 'Яндекс.Музыка',
                  }"
                >
                  {{ tr.source }}
                </span>
                ...
                <div class="shrink-0 text-right">
                  <p class="text-sm font-bold text-white">{{ tr.amount }} ₽</p>
                  <p class="text-xs font-medium text-emerald-400">{{ tr.growth }}</p>
                </div>
              </div>
            </div>
          </div>
