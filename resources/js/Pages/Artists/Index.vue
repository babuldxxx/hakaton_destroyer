...
const props = defineProps({
    artists: Array,          // принятые артисты лейбла
    pendingArtists: Array,   // свободные артисты (pending)
    label: Object,           // текущий лейбл
})
...
const invite = (artistId) => {
    router.post(route('artists.invite', artistId), {}, {
        preserveScroll: true,
        onSuccess: () => {
            // Можно обновить список или показать сообщение
        },
    })
}

// Отвязать артиста (удаление связи)
const detach = (artistId) => {
    if (!confirm('Отвязать артиста от лейбла? Все приглашения будут аннулированы.')) return
    router.delete(route('artists.destroy', artistId), {
        preserveScroll: true,
    })
}
...
<template>
    <div class="p-6 md:p-10 max-w-7xl mx-auto">
        ...
        <div class="flex gap-4 mb-8">
          ...
          : 'bg-[#1A1F2B] text-gray-300 border border-[#2D3748]'"
        ...
        <div v-if="activeTab === 'approved'">
            <div v-if="artists.length === 0" class="text-gray-400 py-10 text-center">
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                ...
                >
                    ...
                    <div class="text-sm text-gray-300 truncate mb-2">
                        {{ artist.user?.email }}
                    </div>
        ...
        <div v-if="activeTab === 'pending'">
            <div v-if="pendingArtists.length === 0" class="text-gray-400 py-10 text-center">
